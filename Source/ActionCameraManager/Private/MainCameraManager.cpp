...
void AMainCameraManager::SetPlayer(AActor* Player) {
	if (!HasAuthority()) ServerSetPlayer(Player);
	Players.Push(Player);
}
void AMainCameraManager::ServerSetPlayer_Implementation(AActor* Player) {
	SetPlayer(Player);
}
bool AMainCameraManager::ServerSetPlayer_Validate(AActor* Player) {
	return true;
}
void AMainCameraManager::GetLifetimeReplicatedProps(TArray< FLifetimeProperty >& OutLifetimeProps) const {
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME(AMainCameraManager, Players);
}